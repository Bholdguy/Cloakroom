...
const CLIENT_ACTIONS_TYPE = "core::array::Span::<privacy::actions::ClientAction>";
export class ScreeningCallMockProofProvider extends CallMockProofProvider {
    actionsDecoder = new CallData(PrivacyPoolABI);
    async prove(invocation, blockIdentifier) {
        ...
        const signature = signScreeningAttestation(SCREENING_SIGNER_PRIVATE_KEY, BigInt(chainId), BigInt(depositor), Number(block.timestamp));
     ...
     */
    depositorToScreen(invocation) {
        const innerCalldata = extractExecuteViewCalldata(invocation.calldata);
        if (innerCalldata.length < 3)
            return undefined;
        try {
            const decoded = this.actionsDecoder.decodeParameters(CLIENT_ACTIONS_TYPE, innerCalldata.slice(2));
...
//# sourceMappingURL=screening-mock-proving.js.map