 ...
 */
import { Account, constants, Contract, EDataAvailabilityMode, ETransactionVersion, hash, OutsideExecutionVersion, } from "starknet";
...
const COMPILED_CONTRACT_PATH = join(__dirname, "../../../target/dev/privacy_Privacy.compiled_contract_class.json");
...
export class Devnet {
    ...
    provider;
    setup;
    accountNonces = new AddressMap(() => 0);
    config;
    exitHandler;
    constructor(config) {
    ...
    async initialize() {
        ...
        const spawnedProcess = this.devnet.process;
        this.exitHandler = () => {
        ...
        for (let i = 0; i < this.config.userAccounts; i++) {
            ...
            const keyBytes = new Uint8Array(raw.private_key
                .replace("0x", "")
                .match(/.{1,2}/g)
                .map((byte) => parseInt(byte, 16)));
            userAccounts.push(new Account({ provider: this.provider, address: raw.address, signer: keyBytes }));
        }
        const [alice, bob] = userAccounts;
        // Admin is always the last predeployed account
        const adminRaw = accounts[this.config.userAccounts];
        const adminKeyBytes = new Uint8Array(adminRaw.private_key
            .replace("0x", "")
            .match(/.{1,2}/g)
            .map((byte) => parseInt(byte, 16)));
        const admin = this.wrapAccount(new Account({
        ...
        debugLog("devnet", "initialize", () => Object.entries(this.setup)
        ...
        return new Proxy(account, {
            get: (target, prop, receiver) => {
                ...
                switch (prop) {
                    ...
                    case "deployContract":
                    case "execute":
                        return async (payload, transactionsDetail) => {
     ...
     * Deploy the privacy pool contract
     */
    async deployPrivacyContract(deployer, contractClass, compiledContract) {
     ...
     */
    async executeOutside(callAndProof) {
        if (!this.setup) {
            throw new Error("Devnet not initialized");
        }
        ...
        const outsideTransaction = await this.setup.admin.getOutsideTransaction(callOptions, callAndProof.call, OutsideExecutionVersion.V2);
     ...
     */
    async cleanup(timeoutMs = 10_000) {
        if (!this.devnet)
            return;
        // StarknetDevnet.process is typed as private but exists at runtime.
        const childProcess = this.devnet.process;
 ...
 */
export async function createDevnetTestEnv(devnet, config) {
 ...
 */
export function createCompatibilityAliceTransfers(env) {
