 ...
 */
import { createEmptyRegistry, WarningCode } from "../interfaces.js";
...
export class ActionCompiler {
    ...
    createPool(privateKey, registry, channels, totalChannels) {
        ...
        for (const [addr, channel] of registry?.channels?.entries() ?? []) {
            if (channels?.has(addr))
                continue; // skip channels that were already set up in the previous step
            pool.setupChannel(addr, channel);
        }
    ...
    async resolveRecipientChannels(actions, options, registry, recipientsNeeded) {
        ...
        const { channels } = await this.discoveryProvider.discoverChannels(this.userAddress, this.userViewingKey, recipientsToDiscover, { blockIdentifier: options?.provingBlockId });
        // see if all recipients were discoveredall
        if (this.allOpen(channels, recipientsToDiscover)) {
            return { channels, total: undefined };
        }
        const { total } = await this.discoveryProvider.discoverChannels(this.userAddress, this.userViewingKey, "total-only", { blockIdentifier: options?.provingBlockId });
    ...
    async resolveNotes(actions, registry, options) {
        ...
        for (const token of balances.keys()) {
            ...
            if (balance < 0n) {
                ...
                throw new Error(`Insufficient balance for token ${toHex(token)}: ` +
